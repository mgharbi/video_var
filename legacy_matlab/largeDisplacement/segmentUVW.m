...
function [uvw, uvw_t, uvw_xform] = segmentUVW(videoA, videoB, top_uvw, top_xform, use_affine)
    ...
    if Ktrans == 1
        ...
        uvw_t = ones(height,width,nF,3);
        uvw_t(:,:,:,1) = uvw_t(:,:,:,1)*top_uvw(1,1);
        uvw_t(:,:,:,2) = uvw_t(:,:,:,2)*top_uvw(1,2);
        uvw_t(:,:,:,3) = uvw_t(:,:,:,3)*top_uvw(1,3);
    elseif Ktrans > 0
        [x,y,t] = meshgrid(1:width, 1:height, 1:nF);
        error_map = zeros(Ktrans, height*width*nF, 'int32');
        ...
        for i = 1 : Ktrans
            ...
            warped = zeros(height, width, nF, 3);
            ...
            t_error_map = (videoA-warped).^2;
            t_error_map = sum(t_error_map,4);
        ...
        h = GCO_Create(height*width*nF,Ktrans);
        GCO_SetDataCost(h,error_map); % data term
        GCO_SetSmoothCost(h,cost_matrix);

        i_x_neighbor = sub2ind([height, width, nF],...
                        y(:,1:end-1,:),...
                        x(:,1:end-1,:),...
                        t(:,1:end-1,:));
        j_x_neighbor = sub2ind([height, width, nF],...
                        y(:,2:end,:),...
                        x(:,2:end,:),...
                        t(:,2:end,:));
        i_y_neighbor = sub2ind([height, width, nF],...
                        y(1:end-1,:,:),...
                        x(1:end-1,:,:),...
                        t(1:end-1,:,:));
        j_y_neighbor = sub2ind([height, width, nF],...
                        y(2:end,:,:),...
                        x(2:end,:,:),...
                        t(2:end,:,:));
        i_t_neighbor = sub2ind([height, width, nF],...
                        y(:,:,1:end-1),...
                        x(:,:,1:end-1),...
                        t(:,:,1:end-1));
        j_t_neighbor = sub2ind([height, width, nF],...
                        y(:,:,2:end),...
                        x(:,:,2:end),...
                        t(:,:,2:end));

        is = cat(1,i_x_neighbor(:), i_y_neighbor(:), i_t_neighbor(:));
        js = cat(1,j_x_neighbor(:), j_y_neighbor(:), j_t_neighbor(:));
        weights = ones(size(is));

        S = sparse(is,js,weights,height*width*nF, height*width*nF);
        ...
        uvw_label=reshape(uvw_label,[height width nF]);
        time = toc;
        fprintf('done. %fs\n',time);

        % imagesc(uvw_label);

        u = zeros(height,width,nF);
        v = zeros(height,width,nF);
        w = zeros(height,width,nF);
        for i=1:Ktrans
    ...
    if Kxform == 1
        ...
        uvw_xform = ones(height,width,nF,3);
        [x,y,t] = meshgrid(1:width, 1:height, 1:nF);
        if use_affine
    ...
    elseif Kxform > 0
        [x,y,t] = meshgrid(1:width, 1:height, 1:nF);
        error_map = zeros(Kxform, height*width*nF, 'int32');

        % Build error maps
        fprintf('   - building error maps...\n');
        uuu = zeros(Kxform,height*width*nF);
        vvv = zeros(Kxform,height*width*nF);
        www = zeros(Kxform,height*width*nF);
        for i = 1 : Kxform
        ...
        h = GCO_Create(height*width*nF,Kxform);
        GCO_SetDataCost(h,error_map); % data term
        GCO_SetSmoothCost(h,cost_matrix);
